// --- Network
resource "oci_core_vcn" "edge_vcn" {
    compartment_id = oci_identity_compartment.edge_comp.id
    cidr_block     = var.vcn_cidr
    display_name   = "vcn-${local.workspace}"
}

resource "oci_core_default_security_list" "edge_default_security_list" {
   manage_default_resource_id = oci_core_vcn.edge_vcn.default_security_list_id
   compartment_id = oci_identity_compartment.edge_comp.id
...
resource "oci_core_security_list" "edge_priv_subnet_security_list" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
...
resource "oci_core_security_list" "edge_adm_security_list" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
...
resource "oci_core_default_route_table" "edge_default_routing_table" {
   # empty default routing table
   manage_default_resource_id = oci_core_vcn.edge_vcn.default_route_table_id
   compartment_id = oci_identity_compartment.edge_comp.id
   display_name = "default_routing_table"
}

resource "oci_core_internet_gateway" "edge_internet_gw" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "internet-gw-${local.workspace}"
}

resource "oci_core_nat_gateway" "edge_nat_gw" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "nat-gw-${local.workspace}"
}

data "oci_core_services" "edge_all_oci_services" {
  filter {
    name   = "name"
...
resource "oci_core_service_gateway" "edge_service_gw" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "service-gw-${local.workspace}"
   services {
      service_id = lookup(data.oci_core_services.edge_all_oci_services[0].services[0], "id")
   }
}

resource "oci_core_route_table" "edge_pub_subnet_rt" {
   #depends_on = [  data.dns_a_record_set.edge_mongodb_dns_records ]
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "pub-subnet-rt-${local.workspace}"
   route_rules {
      network_entity_id = oci_core_internet_gateway.edge_internet_gw.id
      description = "all destinations"
      destination = "0.0.0.0/0"
   }
}

resource "oci_core_route_table" "edge_priv_subnet_rt" {
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "priv-subnet-rt-${local.workspace}"
   route_rules {
      network_entity_id = oci_core_nat_gateway.edge_nat_gw.id
      description = "all destinations"
      destination = "0.0.0.0/0"
   }
   route_rules {
      network_entity_id = oci_core_service_gateway.edge_service_gw.id
      description = "traffic to OCI services"
      destination =  lookup (data.oci_core_services.edge_all_oci_services[0].services[0], "cidr_block")
      destination_type = "SERVICE_CIDR_BLOCK"
   }
}

resource "oci_core_subnet" "edge_adm_subnet" {
   cidr_block     = var.vcn_adm_cidr
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "adm-subnet-${local.workspace}"
   route_table_id = oci_core_route_table.edge_pub_subnet_rt.id
   security_list_ids = [ oci_core_security_list.edge_adm_security_list.id ]
}

resource "oci_core_subnet" "edge_pub_subnet" {
...
resource "oci_core_subnet" "edge_priv_subnet" {
   cidr_block     = var.vcn_priv_cidr
   compartment_id = oci_identity_compartment.edge_comp.id
   vcn_id         = oci_core_vcn.edge_vcn.id
   display_name   = "priv-subnet-${local.workspace}"
   route_table_id = oci_core_route_table.edge_priv_subnet_rt.id
   security_list_ids = [ oci_core_security_list.edge_priv_subnet_security_list.id  ]
}


# --- Outputs
output "edge_nat_gw_ip" {
   value = oci_core_nat_gateway.edge_nat_gw.nat_ip
}
